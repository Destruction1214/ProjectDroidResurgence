...
function MissionUtil.SelectReward(player, tag, tier)
	...
	if ProteusLibrary[group].CustomRewards then
		MasterUnitTable = require("eawx-plugins/intervention-missions/rewards/RewardTables_"..group)
	end
	--/
...
function MissionUtil.HighlightObject(should_highlight, object)
	local highlight_string = ""
	if should_highlight == true then
		
		object.Highlight(true)

		if object.Get_Hint() ~= nil then
			highlight_string = "Highlight_"..object.Get_Hint()
		else
			highlight_string = "Highlight_"..object.Get_Type().Get_Name()
		end

		Add_Radar_Blip(object, highlight_string)
	else 
		...
		Remove_Radar_Blip(highlight_string)
	end
end
...
function MissionUtil.ToggleAbility(player,unit_name,ability_name,state)
    ...
    local plot = Get_Story_Plot("Conquests\\Factions\\Story_Generic_Space_"..player_name..".xml")
    if not plot then
        return
    end
...
function MissionUtil.Find_Attacking_Player()
	DebugMessage("%s -- Entering MissionUtil.Find_Attacking_Player", tostring(Script))
	...
	if not TestValid(attacker_marker) then
		attacking_player = Find_Player("Hostile")
	else
		attacking_player = attacker_marker.Get_Owner()
		if not TestValid(attacking_player) then
			attacking_player = Find_Player("Hostile")
		end
	end

	DebugMessage("%s -- Exiting MissionUtil.Find_Attacking_Player", tostring(Script))

	return attacking_player
end

function MissionUtil.Find_Defending_Player()
	DebugMessage("%s -- Entering MissionUtil.Find_Defending_Player", tostring(Script))
	...
	if not TestValid(defender_marker) then
		...
		defending_player = defender_marker.Get_Owner()
		if not TestValid(defending_player) then
			defending_player = Find_Player("Hostile")
		end
	end

	DebugMessage("%s -- Exiting MissionUtil.Find_Defending_Player", tostring(Script))
